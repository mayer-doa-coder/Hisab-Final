...
- The undone entry stays in the ledger, softened and marked "Undone", beside the entry that cancelled it, and offers no second undo. A credit sale's baki offers no undo here at all: it is undone with its sale and stock from History (D021).
- `BakiRepository.reverse` decides "has this been undone already?" from stored rows, in the same transaction as the write. Proved by breaking it: with the check removed, a second undo went through and eight undos fired at once wrote eight opposite entries instead of one.
- The balance the dialog promises is `balanceAfterUndo`, tested to equal what the written entry produces for every entry kind. If undoing leaves the customer ahead, it reads "Paid in advance", never a negative "owes".
- Step 57's own check, on the phone: 500 in baki, 200 received, 100 more (400), undo the payment - the balance reads 600, both entries are still in the ledger, and the stored entries add up to 60000 poisha; then undo the 100 - 500.
- Tests: 6 new Android unit tests (198 in total, 0 failures); 9 new repository tests and 1 new flow test on the phone (191 on-device in total, 0 failed, 0 skipped).

### Added (M3 — Customer and baki screens, Steps 52-56)
- Customer List screen (Step 52): every customer with what they owe, ordered by who most needs a phone call - overdue first, then the largest baki, then name. A card above the list totals what is owed (an advance from one customer never reduces what another owes), with the count of who owes and who is overdue. Filters (everyone / owe money / overdue) and a search over name or phone sit above it. Adding a customer whose name is already taken is reported, with the existing person one tap away, rather than a second record being made or the two silently merged (D035, D042).
- Customer Details screen (Step 53): the balance, worked out from that customer's baki entries and never stored (D001), with an overdue banner when something is past its due date. The ledger below it is newest first, and each line shows the balance right after it, so the top line's number is the same one shown above - there is no second place for the two to disagree.
- Add Baki and Receive Payment (Steps 54-55): one screen in two modes, sharing the same view model as the customer's own screen, so the "owes now -> will owe" preview is the live balance, not a copy that could go stale. The amount keypad is up as the screen opens. A full-amount payment is one tap. Saving writes locally and returns to the customer's screen, where the new balance and a confirmation are the answer to "did that work". Nothing here touches the network - the same as every other write in the app.
- Overpaying is recorded, never blocked (D041): the screen says the extra will be kept as an advance, and an advance is shown everywhere as an amount held ("Paid in advance TK1,000.00"), never as a negative "owes" number.
- Step 56's exact fixture, run on the real screens on a real phone: 500 in baki, 200 received, 100 more added, showing TK400.00 - and the stored entries add up to the same 40000 poisha.
- `domain/Baki.kt`'s functions now reach the screens through `BakiRepository` and `CustomerRepository.create`, both of which write locally and queue nothing for a hand-written baki entry yet (see Fixed/Not done below).
- `whenText` and the customer screens show dates and times in the digits of the screen's language, matching how money is already shown (D042).
- Tests: 45 new Android unit tests (192 in total, 0 failures) covering list ordering and filtering, the ledger's running balance, and the amount and phone parsing; 6 new instrumented tests through the real screens (35 total for Steps 50-56 across CustomerDaoTest, BakiEntryDaoTest and CustomerFlowTest, all passing on the Galaxy A15), including the exact Step 56 numbers and a past due date making a customer overdue everywhere it is shown.
- `DECISIONS.md` D042: why baki entered here is not queued for sync yet, the list's order, the shared view model, how an advance is shown, and the taken-name answer.

### Not done here, on purpose
- Baki added or paid on these screens is not queued for sync (D042, extending D036): the server has no endpoint for a standalone baki entry until Step 58. It stays on the phone until then.
- The Postgres `baki_entry_type_known` check still allows only `credit_sale` and `reversal` - widening it belongs with Step 58, and must land before a hand-written credit can sync.
